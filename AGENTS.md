...
PATCH and PUT business-rule failures use `BookValidationException`. Its dedicated global handler
uses the shared validation-response builder, keeping book validation distinct from unrelated
argument errors while preserving the public HTTP 400 response shape.
...
- `UserServiceTest`: 11 Mockito-based service unit tests for partial-update normalization, validation, password verification and encoding, unchanged-email handling, duplicate-email rejection, and dirty-checking expectations.
...
The suite contains 80 tests. Its execution setup is deliberately small and optimized:
