...
| 3.3 | **Timestamp uses epoch milliseconds** | `ApiResponse`, `ErrorResponse` — `System.currentTimeMillis()` | `timestamp` field is a raw epoch-long, not ISO-8601 | Epoch milliseconds are machine-readable but not human-readable. ISO-8601 would align with JSON conventions and improve developer ergonomics. |
...
| 3.8 | **Genre distribution returns bare `Map<String, Long>`** | `BookAPI.getGenre` | A raw `Map` with no envelope, no ordering guarantee, and no pagination | This is the most inconsistent response shape in the API — a bare map while every other endpoint wraps data. |
...
| # | Gap | Location | Impact | Why it matters |
|---|-----|----------|--------|-----------------|
| 5.2 | **No `CHECK (price > 0)` constraint** | V1 migration (`price DECIMAL(10,2) NOT NULL`) | The database accepts zero or negative prices if data is inserted directly | AGENTS.md explicitly documents this: "The database enforces `NOT NULL` for price but not a positive-value check; application validation is the current positive-price guard." If the application is bypassed (direct DB access, migration, bulk import), invalid prices enter the system. |
...
| # | Gap | Location | Impact | Why it matters |
|---|-----|----------|--------|-----------------|
| 6.1 | **Dockerfile is not multi-stage and requires pre-built JAR** | `Dockerfile` — `COPY target/*.jar app.jar` | `docker compose build` fails if `mvn package` hasn't been run first | AGENTS.md notes: "The Dockerfile is not a multi-stage build." This creates a two-step build process that is error-prone for CI/CD pipelines and new contributors. A multi-stage Dockerfile would `mvn package` in the builder stage and copy the resulting JAR. |
...
| # | Gap | Location | Impact | Why it matters |
|---|-----|----------|--------|-----------------|
| 8.1 | **Resolved: typed genre aggregation projection** | `BookRepository.getGenres()` returns `List<GenreCount>` | Genre rows are mapped through named `genre()` and `count()` components; no positional indexes or runtime casts are required | `GenreCount` now applies the same immutable constructor-projection pattern as `LibraryAggregate`. The public `Map<String, Long>` response remains unchanged. A persistence-level integration test is still pending because the repository has no JPA/PostgreSQL integration-test setup. |
